...
public class Sort {
   ...
   long executionTime = 0;
    ...
    void merge(int list[],int low,int middle,int high){
        int i,j,k;
        int leftArrayLenght = middle-low+1;
        int rightArrayLength = high-middle;
        int[]leftArray=new int[leftArrayLenght];
        int[]rightArray=new int[rightArrayLength];
        for(i=0;i<leftArrayLenght;i++)
            leftArray[i]=list[low+1];
        for(j=0;j<rightArrayLength;j++)
            rightArray[j]=list[middle+1+j];
        i=0;
        j=0;
        k=low;
        while(i<leftArrayLenght && j<rightArrayLength){
            if(leftArray[1]<=rightArray[j]){
                list[k]=leftArray[i];
                i++;
            }
            else{
